...
public class ParabolaController : MonoBehaviour
{
    ...
    void SetAnim(bool isok)
    {
        anim.AnimControl("isMove", !isok);
        anim.AnimControl("isFly", isok);
    }
    ...
    void Update()
    {
        ...
        if (Vector3.Distance(
            ParabolaRoot.transform.Find("Ust").transform.position, transform.position) < 0.5f)
        {

            Speed = 5f;
            SetAnim(false);
            GetComponent<CharacterControl>().enabled = true;
            GetComponent<CharacterControl>().isStop = false;
    ...
    public class ParabolaFly
    {
        ...
        public ParabolaFly(Transform ParabolaRoot)
        {
            ...
            List<Component> components = new List<Component>(ParabolaRoot.
